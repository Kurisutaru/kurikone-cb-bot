...
class SetupCommands(
    commands.Cog, name="Setup Commands", description="Collection of Setup Commands"
):
    ...
    async def sc_install(self, interaction: discord.Interaction):
        ...
        async def button_ok_callback(interact: discord.Interaction):
            ...
            setup_result = await _main_service.install_bot_command(
            ...
            await utils.send_followup_short(
                interaction=interact,
                content=l.t(guild_id, "message.done_install"),
                ephemeral=True,
            )
    ...
    async def sc_uninstall(self, interaction: discord.Interaction):
        ...
        async def button_ok_callback(interact: discord.Interaction):
            await interact.response.defer(thinking=True, ephemeral=True)
            guild = interaction.guild
            action = await _main_service.uninstall_bot_command(
                guild, TL_SHIFTER_CHANNEL
            )
            ...
            await utils.send_followup_short(
                interaction=interact,
                content=l.t(guild_id, "message.done_uninstall"),
                ephemeral=True,
            )
